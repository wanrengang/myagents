    ...
    <div class="form-panel">
      ...
      <div class="groups">
        <div v-for="g in groupDefs" :key="g.key" class="group">
          ...
          <div class="chips">
            <span v-if="!catalog[g.key]?.length" class="hint-text">（暂无可选项）</span>
            <label
              v-for="it in (catalog[g.key] || [])" :key="it.id"
              class="chip"
            ...
            >
              ...
              <span>{{ it.name }}</span>
              <span v-if="it.description" class="chip-desc">{{ it.description }}</span>
            </label>
          </div>
        </div>
        <div class="hint-text">提示：勾选「start_refund」工具会自动启用人工审批；勾选知识库后该员工的 kb_search 只检索所选库。</div>
      </div>
...
const groupDefs = [
  ...
  { key: 'knowledge_bases', label: '知识库 Knowledge Bases' },
...
function loadEmp(id) {
  ...
  selected.knowledge_bases = new Set(e.kbs || [])
