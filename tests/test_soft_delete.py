...
def test_user_soft_delete():
    ...
    rows = _raw(catalog.DB, "SELECT * FROM users WHERE id=?", (uid,))
...
def test_skill_soft_delete_and_links_hard_delete():
    ...
    rows = _raw(catalog.DB, "SELECT * FROM skills WHERE id=?", ("sd_skill",))
    assert len(rows) == 1 and rows[0]["deleted_at"]
    # 关联边硬删（employee_skills 里没有了）
    assert _raw(catalog.DB, "SELECT * FROM employee_skills WHERE skill_id=?", ("sd_skill",)) == []
    # 员工 effective config 不再含该技能
    cfg = catalog.get_employee_config(emp_id)
    assert "sd_skill" not in (cfg.get("skills") or [])


def test_kb_soft_delete_cascades_entries():
    catalog.create_kb("sd_kb", "软删库")
    catalog.create_kb_entry("sd_kb", "sd_e1", "条目1", ["k"], "内容")
    catalog.create_employee({"id": "sd_emp2", "name": "E2", "kbs": ["sd_kb"]})
    affected = catalog.delete_kb("sd_kb")
    assert "sd_emp2" in affected
    # 库与条目读取不可见
    assert catalog.list_kb_entries("sd_kb") == []
    # 物理行保留且被标记
    kb_rows = _raw(catalog.DB, "SELECT * FROM knowledge_bases WHERE id=?", ("sd_kb",))
    en_rows = _raw(catalog.DB, "SELECT * FROM kb_entries WHERE id=?", ("sd_e1",))
    assert kb_rows[0]["deleted_at"] and en_rows[0]["deleted_at"]
    # 关联边硬删
    assert _raw(catalog.DB, "SELECT * FROM employee_kbs WHERE kb_id=?", ("sd_kb",)) == []
...
def test_sop_connector_employee_soft_delete():
    ...
    for table, id_ in (("sops", "sd_sop"), ("connectors", "sd_conn"), ("employees", "sd_emp3")):
        rows = _raw(catalog.DB, f"SELECT * FROM {table} WHERE id=?", (id_,))
        assert len(rows) == 1 and rows[0]["deleted_at"], table
    # 员工的关联边全部硬删
    for table in ("employee_sops", "employee_connectors"):
        assert _raw(catalog.DB, f"SELECT * FROM {table} WHERE employee_id=?", ("sd_emp3",)) == []
