...
def get_current_time() -> str:
    """获取当前真实的日期与时间（东八区 / 北京时间）。

    当用户问到“今天几号”“现在几点”“本周/本月/今年是哪周”“距离某个日期
    还有多久”“截止日期是哪天”或任何需要知道当前真实时间的问题时，
    **必须先调用本工具**再回答，不要凭记忆或训练数据猜测日期。

    返回内容含：年月日、星期几、当前时分秒、ISO 日期与所在周数、
    Unix 时间戳，以及对应的 UTC 参考时间，便于跨时区换算。"""
    now = datetime.now(TZ)
